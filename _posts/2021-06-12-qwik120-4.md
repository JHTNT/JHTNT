...
[運算地區](https://cloud.google.com/compute/docs/regions-zones/#available)（compute zone）是你的叢集及其資源在區域的概略位置，如 `us-central1-a` 是 `us-central1` 的其中一個地區。

要設置預設運算地區，先**在 Cloud Shell 開啟新會話**，然後運行以下指令：
