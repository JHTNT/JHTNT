...
關於區域和地區在[上一篇](/articles/20210603-qwik120-2#認識區域和地區)有說明。若要查看預設區域和地區可以使用以下指令：
...
從輸出中找到區域和地區的元資料（metadata，描述其他資料的資料）值，在稍後的實驗會使用到地區（`google-compute-default-zone`）。
...
2. 建立儲存地區的環境變數，將 `<zone>` 替換為 `gcloud compute project-info describe` 指令輸出裡的 ***zone* 值**：
...
- `--zone` flag 指定虛擬機建立的地區。

<div markdown="span" class="info">
如果省略 `--zone`，`gcloud` 會根據你的預設屬性選擇區域。若是省略其他必要設定如 `machine type` 與 `image`，則會設為預設值。
</div>
