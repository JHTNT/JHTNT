...
位於區域中的資源稱為區域性資源，虛擬機執行個體和永久性磁碟位於一個區域中。若要將永久性磁盤附加到虛擬機執行個體（VM instance），**兩者必須在同一區域中**。

相似地，如果要為執行個體分配靜態 IP 地址，則兩者必須在**同一地區**。
...
4. 也可以使用 SSH，透過 `gcloud` 連接到執行個體。若有區域預設值，則可忽略 `--zone` 參數：
